...
class ZigbeeHydroponicsController extends ZigBeeDevice {
  ...
  async onNodeInit({ zclNode }) {
    ...
    this.registerCapability('auxiliary_output', CLUSTER.ON_OFF, {
      endpoint: 1,
  ...
  onPressureMeasuredAttributeReport(measuredValue) {
    this.log('Received pH value ', measuredValue);
    const parsedValue = Math.round(measuredValue) / 10;
    this.log('measure_ph: ', parsedValue);
    this.setCapabilityValue('measure_ph', parsedValue).catch(this.error);
  }

  onFlowMeasuredAttributeReport(measuredValue) {
    this.log('Received conductivity value ', measuredValue);
    const parsedValue = Math.round(measuredValue) / 10;
    this.log('measure_conductivity: ', parsedValue);
    this.setCapabilityValue('measure_conductivity', parsedValue).catch(this.error);
  }

}

module.exports = ZigbeeHydroponicsController;
